...
def start_extraction_thread(video_path, num_frames, output_dir, progress_label):
    ...
    def update_progress(progress):
        progress_label.after(0, lambda: progress_label.config(text=f"Progress: {int(progress * 100)}%"))

    threading.Thread(target=extract_frames, args=(video_path, num_frames, output_dir, update_progress), daemon=True).start()
...
def start_extraction(path_entry, frame_count_entry, output_dir_entry, progress_label):
    ...
    try:
        ...
        messagebox.showwarning("Warning", "Please enter a valid number of frames.")
        return
